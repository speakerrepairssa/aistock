...
export const useRepairStore = create<RepairStore>((set, get) => ({
  ...
  updateRepairJob: async (id: string, data: Partial<RepairJob>) => {
    try {
      // Get the job from state to find the userId
      const job = get().repairJobs.find((j) => j.id === id);
      if (!job) throw new Error('Repair job not found');
      
      // Pass the userId in data for the service to use
      await repairService.updateRepairJob(id, { ...data, createdBy: job.createdBy });
      set((state) => ({
  ...
  deleteRepairJob: async (id: string) => {
    try {
      ...
      await repairService.deleteRepairJob(id, job.createdBy);
      set((state) => ({
