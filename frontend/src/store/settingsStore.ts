...
export const useSettings = create<SettingsStore>((set, get) => ({
  ...
  initializeSettings: async (userId: string) => {
    ...
    try {
      ...
      // Convert Firebase settings to store format
      const integrations: Integration[] = [];
      if (userSettings.integrations.clickup) {
        integrations.push({
          name: 'ClickUp',
          apiKey: userSettings.integrations.clickup.apiKey,
          enabled: userSettings.integrations.clickup.enabled,
          teamId: userSettings.integrations.clickup.teamId,
        });
      }
      if (userSettings.integrations.cloudinary) {
        integrations.push({
          name: 'Cloudinary',
          apiKey: userSettings.integrations.cloudinary.apiKey,
          enabled: userSettings.integrations.cloudinary.enabled,
        });
      }
  ...
  updateLocalSettings: (settings: Partial<UserSettings>) => {
    ...
    if (settings.integrations) {
      const integrations: Integration[] = [];
      if (settings.integrations.clickup) {
        integrations.push({
          name: 'ClickUp',
          apiKey: settings.integrations.clickup.apiKey,
          enabled: settings.integrations.clickup.enabled,
          teamId: settings.integrations.clickup.teamId,
        });
      }
      if (settings.integrations.cloudinary) {
        integrations.push({
          name: 'Cloudinary',
          apiKey: settings.integrations.cloudinary.apiKey,
          enabled: settings.integrations.cloudinary.enabled,
        });
      }
