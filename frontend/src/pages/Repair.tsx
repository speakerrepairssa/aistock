...
export const Repair: React.FC = () => {
  ...
  const { currency, technicians, addTechnician, removeTechnician, integrations, addIntegration, updateIntegration, removeIntegration, formFields, addFormField, updateFormField, removeFormField, resetFormFields, initializeSettings } = useSettings();
  ...
  }, [user?.uid, fetchRepairJobs, fetchProducts, initializeSettings]);
  ...
  const handleFetchClickupTasks = async (teamId?: string) => {
    ...
    console.log('[Repair] handleFetchClickupTasks called with teamId:', teamIdToUse);
    console.log('[Repair] integrations:', integrations);
    ...
    try {
      // Get the API key from the current integration
      const clickupIntegration = integrations?.find(i => i.name.toLowerCase() === 'clickup');
      const apiKey = clickupIntegration?.apiKey;
      
      console.log('[Repair] Calling clickupService.getTeamTasks...');
      console.log('[Repair] Passing API key:', apiKey);
      const tasks = await clickupService.getTeamTasks(teamIdToUse, apiKey);
      console.log('[Repair] Tasks received:', tasks.length);
      ...
      if (currentJobNumberForClickup && currentJobNumberForClickup.trim()) {
        ...
        const matchingTaskIds = tasks
          .filter(task => {
            // Make comparison more robust - handle string/number and trim whitespace
            const taskJobNumber = task.jobNumber ? String(task.jobNumber).trim() : '';
            return taskJobNumber === jobNumberToMatch || taskJobNumber === String(jobNumberToMatch);
          })
          .map(task => task.id);
        
        console.log(`Found ${matchingTaskIds.length} tasks matching job number: ${jobNumberToMatch}`);
        setSelectedClickupTasks(matchingTaskIds);
        
        // Show feedback to user
        if (matchingTaskIds.length > 0) {
          console.log(`✓ Automatically selected ${matchingTaskIds.length} task(s) matching job #${jobNumberToMatch}`);
        } else {
          console.log(`⚠ No tasks found with job number: ${jobNumberToMatch}`);
        }
      } else {
        setSelectedClickupTasks([]);
      }
  ...
  const handleDragStart = (product: any) => {
    console.log('[Repair] Drag started for product:', product.name);
    ...
    console.log('[Repair] Dragged product set:', item);
  };

  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    console.log('[Repair] Drag over card');
  ...
  const handleDropOnJob = async (e: React.DragEvent<HTMLDivElement>, jobId: string) => {
    e.preventDefault();
    console.log('[Repair] Drop on job:', jobId);
    console.log('[Repair] Dragged product:', draggedProduct);
    (e.currentTarget as HTMLDivElement).style.backgroundColor = 'white';

    if (draggedProduct) {
      try {
        console.log('[Repair] Adding product to job...');
        await addProductToJob(jobId, draggedProduct);
        console.log('[Repair] Product added successfully');
        setDraggedProduct(null);
      } catch (error) {
        console.error('Error adding product to job:', error);
      }
    } else {
      console.warn('[Repair] No dragged product to add!');
    }
  };
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {showPutAsideJobs && <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid item xs={12} sm={6} md={3} key={index}>4
            {job ? (
              <Card
                ...
                onDragLeave={handleDragLeave}
                onDrop={(e) => {
                  // Check if it's a product being dropped or a job
                  const jobId = e.dataTransfer.getData('jobId');
                  if (jobId) {
                    // It's a job being moved back to repair
                    handleDropJobBackToRepair(e);
                  } else {
                    // It's a product being added
                    handleDropOnJob(e, job.id);
                  }
                }}
                sx={{
                  minHeight: 300,
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <TextField
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && newTechnicianName.trim()) {
                      addTechnician(user!.uid, newTechnicianName.trim());
                  ...
                  {technicians.map((tech) => (
                    ...
                    >
                      ...
                      <Button
                        size="small"
                        color="error"
                        onClick={() => removeTechnician(user!.uid, tech)}
                      >
                        Remove
                      </Button>
                    </Box>
            ...
            <Box sx={{ borderTop: '1px solid #ddd', pt: 2, mb: 3 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Button size="small" variant="outlined" onClick={() => resetFormFields(user!.uid)}>
                    Reset to Default
                  </Button>
                  <Button
                    size="small"
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (newFieldLabel.trim()) {
                            if (editingField) {
                              updateFormField(user!.uid, editingField.id, {
                                label: newFieldLabel,
                                type: newFieldType,
                              });
                            } else {
                              addFormField(user!.uid, {
                                id: '',
                  ...
                  >
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      ...
                      <Button
                        ...
                        color="error"
                        variant="outlined"
                        onClick={() => removeFormField(user!.uid, field.id)}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (newIntegrationName && newIntegrationKey && hasTeamId) {
                            addIntegration(user!.uid, newIntegrationName, newIntegrationKey, newIntegrationTeamId || undefined);
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        <Button
                          ...
                          onClick={() =>
                            updateIntegration(user!.uid, integration.name, integration.apiKey, !integration.enabled)
                          }
                        >
                        ...
                        <Button
                          size="small"
                          color="error"
                          onClick={() => removeIntegration(user!.uid, integration.name)}
                        >
                          Remove
                        </Button>
                      </Box>
        ...
        <DialogContent sx={{ py: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {loadingClickupTasks ? (
              ...
              const clickupIntegration = integrations?.find(i => i.name.toLowerCase() === 'clickup');
              console.log('[Repair] Dialog opened - integrations:', integrations);
              console.log('[Repair] Dialog opened - clickupIntegration:', clickupIntegration);
              console.log('[Repair] Dialog opened - clickupTasks.length:', clickupTasks.length);
              const isConfigured = clickupIntegration?.enabled && clickupIntegration?.teamId;
              console.log('[Repair] Dialog opened - isConfigured:', isConfigured);
              return isConfigured || clickupTasks.length > 0;
            })() ? (
              <>
                {/* Job Number Filter */}
                <Box sx={{ mb: 2, p: 2, backgroundColor: '#f0f9ff', borderRadius: 1, border: '1px solid #0284c7' }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    ...
                    <Button
                      ...
                      size="small"
                      onClick={() => {
                        console.log('[Repair] Load Tasks button clicked');
                        const clickupIntegration = integrations?.find(i => i.name.toLowerCase() === 'clickup');
                        console.log('[Repair] Found integration:', clickupIntegration);
                        if (clickupIntegration?.enabled && clickupIntegration?.teamId) {
                          console.log('[Repair] Integration is enabled, calling handleFetchClickupTasks');
                          setClickupTeamId(clickupIntegration.teamId);
                          handleFetchClickupTasks(clickupIntegration.teamId);
                        } else {
                          console.log('[Repair] Integration not properly configured');
                          alert('ClickUp integration is not properly configured. Please go to Settings > Integrations.');
                        }
                      }}
                ...
                {/* Show feedback when tasks are auto-selected */}
                {selectedClickupTasks.length > 0 && currentJobNumberForClickup && (
                  <Alert severity="success" sx={{ mb: 1 }}>
                    ✓ Auto-selected {selectedClickupTasks.length} task{selectedClickupTasks.length !== 1 ? 's' : ''} matching Job #{currentJobNumberForClickup}
                  </Alert>
                )}
                ...
                {clickupTasks.length > 0 ? (
                  ...
                  <Box sx={{ p: 2, textAlign: 'center', backgroundColor: '#f0f9ff', borderRadius: 1, border: '1px solid #0284c7' }}>
                    <Typography variant="body2" color="textSecondary" sx={{ mb: 1 }}>
                      Click "Load Tasks" button above to fetch tasks from ClickUp
                    </Typography>
                    <Typography variant="caption" color="textSecondary">
                      Enter a job number to automatically select matching tasks, or leave empty to see all tasks
                    </Typography>
                  </Box>
                )}
              </>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {(() => {
                  const clickupIntegration = integrations?.find(i => i.name.toLowerCase() === 'clickup');
                  const hasApiKey = !!clickupIntegration?.apiKey;
                  const isEnabled = !!clickupIntegration?.enabled;
                  const hasTeamId = !!clickupIntegration?.teamId;
                  
                  console.log('[Repair] Warning view - hasApiKey:', hasApiKey);
                  console.log('[Repair] Warning view - isEnabled:', isEnabled);
                  console.log('[Repair] Warning view - hasTeamId:', hasTeamId);
                  
                  return (
                    <>
                      <Alert severity="warning">
                        ⚠️ ClickUp integration not configured. 
                        {!clickupIntegration && ' No integration found.'}
                        {clickupIntegration && !hasApiKey && ' API Key missing.'}
                        {clickupIntegration && hasApiKey && !hasTeamId && ' Team ID missing.'}
                        {clickupIntegration && hasApiKey && hasTeamId && !isEnabled && ' Integration disabled.'}
                      </Alert>
                    </>
                  );
                })()}
                <Button
                  ...
                  sx={{ mb: 2 }}
                >
                  Open Settings to Configure ClickUp
                </Button>
                <Box sx={{ p: 2, backgroundColor: '#f0f9ff', borderRadius: 1, border: '1px solid #0284c7' }}>
                  ...
                  <ol style={{ marginTop: '8px', marginBottom: 0, paddingLeft: '20px' }}>
                    <li>Click the button above (or click Settings gear icon)</li>
